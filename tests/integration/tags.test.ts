...
describe('Tags entry count query', () => {
  ...
  it('returns accurate entry count for tags with associated entries', async () => {
    const [tag1] = await db.insert(schema.tags).values({
      userId: testUserId,
      name: 'Alpha',
      color: '#4F46E5',
    }).returning()

    const [tag2] = await db.insert(schema.tags).values({
      userId: testUserId,
      name: 'Beta',
      color: '#EC4899',
    }).returning()
  ...
  it('returns accurate counts for a mix of tags with and without entries', async () => {
    const [tagA] = await db.insert(schema.tags).values({
      userId: testUserId,
      name: 'Has Entries',
      color: '#22C55E',
    }).returning()
  ...
  it('updates count when an entry-tag association is removed', async () => {
    ...
    const [assoc1] = await db.insert(schema.entryTags).values([
