...
describe('DOCX Generation', () => {
  ...
  it('sorts entries by different criteria', async () => {
    const entries = [
      makeEntry({ id: '1', title: 'Zebra Book', authors: [{ firstName: 'Zara', lastName: 'Zoey' }], year: 2020 }),
      makeEntry({ id: '2', title: 'Alpha Book', authors: [{ firstName: 'Anna', lastName: 'Adams' }], year: 2024 }),
    ]
  ...
  it('handles different entry types in DOCX', async () => {
    const entries = [
      ...
      makeEntry({
        ...
        entryType: 'journal_article',
        title: 'An Article',
        metadata: { journal: 'Nature', volume: '1', issue: '2', pages: '1-10', doi: '10.1234/x' },
      }),
      makeEntry({
        id: '3',
        entryType: 'website',
