...
import {
  ...
  sortEntries,
} from "~/server/services/export/pdf";
...
describe("BibTeX Generation", () => {
  ...
  it("handles multiple authors correctly", () => {
    ...
    expect(result).toContain(
      "author = {Smith, Alice and Jones, Bob and White, Carol}",
    );
  });
...
describe("DOCX Generation", () => {
  ...
  it("includes annotations when option is set", async () => {
    ...
    expect(bufferWithAnnotations.length).toBeGreaterThan(
      bufferWithoutAnnotations.length,
    );
  });
...
describe("sortEntries", () => {
  ...
  it("sorts by year descending", () => {
    const entries = [
      makeEntry({ id: "1", year: 2020 }),
      makeEntry({ id: "2", year: 2024 }),
    ];
  ...
  it("does not mutate the original array", () => {
    const entries = [
      makeEntry({ id: "1", title: "B" }),
      makeEntry({ id: "2", title: "A" }),
    ];
    const sorted = sortEntries(entries, "title");
