...
test.describe("Export Modal - Opening from different pages", () => {
  ...
  test("opens export modal from dashboard quick action", async ({ page }) => {
    ...
    await page
      .getByTestId("dashboard-quick-action-export")
    ...
    await expect(page.getByTestId("export-modal-title")).toHaveText(
      "Export Bibliography",
    );
...
test.describe("Export Modal - Format selection and options", () => {
  ...
  test("switching to DOCX shows same document options as PDF", async ({
    page,
  }) => {
  ...
  test("switching between all formats preserves modal state", async ({
    page,
  }) => {
    await page.getByTestId("export-format-pdf").click();
    await expect(page.getByTestId("export-pdf-options")).toBeVisible({
  ...
  test("PDF and DOCX show tier restriction for free users", async ({
    page,
  }) => {
    await page.getByTestId("export-format-pdf").click();
  ...
  test("BibTeX does not show tier restriction for free users", async ({
    page,
  }) => {
    await page.getByTestId("export-format-bibtex").click();
  ...
  test("Excel does not show tier restriction for free users", async ({
    page,
  }) => {
    await expect(page.getByTestId("export-tier-warning")).not.toBeVisible();
    await expect(page.getByTestId("export-submit-btn")).toBeEnabled();
  ...
  test("entry count displays correctly when no entries selected", async ({
    page,
  }) => {
    await expect(page.getByTestId("export-entry-count")).toContainText(
      "All library entries",
    );
  });
});
...
test.describe("Export Modal - Preview panel", () => {
  test("PDF format shows preview panel with bibliography content", async ({
    page,
  }) => {
    await signUpAndLogin(page);
