...
test.describe('Library - Quick Add Immediate Update', () => {
  ...
  test('entry added via Quick Add appears immediately on library page without refresh', async () => {
    ...
    await expect(sharedPage.getByRole('heading', { name: /Add a source/i })).toBeVisible({ timeout: 5000 })
    
    const input = sharedPage.getByPlaceholder(/Paste DOI, ISBN, URL/i)
    await input.fill(entryTitle)
    
    await sharedPage.waitForResponse(r => r.url().includes('/api/entries/suggest'), { timeout: 10000 }).catch(() => null)
    ...
    try {
      ...
      await expect(sharedPage.getByRole('button', { name: /Add to library/i })).toBeVisible({ timeout: 5000 })
      await sharedPage.getByRole('button', { name: /Add to library/i }).click()
    }
    
    await sharedPage.waitForResponse(r => r.url().includes('/api/entries') && r.request().method() === 'POST', { timeout: 10000 })
    
    await expect(sharedPage.getByRole('heading', { name: /Add a source/i })).toBeHidden({ timeout: 10000 })
  ...
  test('multiple entries added via Quick Add all appear without refresh', async () => {
    ...
    const entries = [
      `Quick Entry One ${timestamp}`,
      `Quick Entry Two ${timestamp + 1}`,
    ]
