...
function recordRequest(userId: string, path: string) {
  ...
  const endpoint = path.replace(/\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/gi, '/:id')
...
export default defineNitroPlugin((nitroApp) => {
  ...
  nitroApp.hooks.hook('afterResponse', (event) => {
    ...
    if (!userId) {
      const session = event.context.sessions as Record<string, { user?: { id?: string } }> | undefined
