...
export default defineEventHandler(async (event) => {
  ...
  const impersonatedBy = (session as any)?.impersonatedBy as { id: string; email: string; name?: string } | undefined
  ...
  await logAdminAction(impersonatedBy.id, 'user.stop_impersonate', 'user', currentUser?.id, {
