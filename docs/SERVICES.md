...
async function formatCitation(entry: Entry, styleId: string): Promise<string>
...
async function ingestEntry(projectId: string, entry: Entry): Promise<void>

// Process uploaded file
async function ingestUpload(projectId: string, upload: ProjectUpload): Promise<void>
...
async function generatePersona(projectId: string): Promise<ResearchPersona>
```
...
```typescript
async function generateExcel(entries: Entry[], options: ExcelOptions): Promise<Buffer>
...
```typescript
async function generatePdf(entries: Entry[], options: PdfOptions): Promise<Buffer>
...
```typescript
async function generateBibtex(entries: Entry[]): Promise<string>
```
...
```typescript
async function buildProjectGraph(projectId: string, options: GraphOptions): Promise<GraphData>
...
// Revoke share
async function revokeShare(shareId: string, ownerId: string): Promise<void>
```
...
async function createPortalSession(userId: string): Promise<string> // Returns portal URL

// Handle webhook events
async function handleWebhook(body: string, signature: string): Promise<void>

// Get subscription details
async function getSubscription(userId: string): Promise<SubscriptionDetails | null>

// Cancel subscription
async function cancelSubscription(subscriptionId: string, immediately: boolean): Promise<void>
```
...
```typescript
async function extractMetadata(url: string, options?: ExtractOptions): Promise<ExtractedMetadata>
...
```typescript
async function calculateVeritasScore(entry: Entry): Promise<VeritasScore>
...
// OpenAlex
async function fetchOpenAlexData(doi: string | null, title: string): Promise<OpenAlexResult>

// CrossRef
async function fetchCrossRefData(doi: string): Promise<CrossRefResult>
```
