...
const statCards = computed(() => {
  ...
  return [
    { label: 'Total Users', value: stats.value.users.total, icon: 'i-heroicons-users', color: 'text-blue-500' },
    { label: 'New This Week', value: stats.value.users.newThisWeek, icon: 'i-heroicons-user-plus', color: 'text-green-500' },
    { label: 'New This Month', value: stats.value.users.newThisMonth, icon: 'i-heroicons-calendar', color: 'text-purple-500' },
    { label: 'Active Subscriptions', value: stats.value.subscriptions.active, icon: 'i-heroicons-credit-card', color: 'text-amber-500' },
    { label: 'Total Entries', value: stats.value.content.totalEntries, icon: 'i-heroicons-book-open', color: 'text-cyan-500' },
    { label: 'Total Projects', value: stats.value.content.totalProjects, icon: 'i-heroicons-folder', color: 'text-indigo-500' },
    { label: 'Open Feedback', value: stats.value.feedback.open, icon: 'i-heroicons-inbox', color: 'text-rose-500' },
    { label: 'MRR', value: `$${(stats.value.subscriptions.mrr ?? 0).toLocaleString()}`, icon: 'i-heroicons-banknotes', color: 'text-emerald-500' },
  ]
})
  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        ...
        >
          <span class="w-2 h-2 rounded-full" :class="health?.status === 'healthy' ? 'bg-green-500' : 'bg-red-500'" />
    ...
    <UCard>
      <template #header>
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Subscription Distribution</h2>
      </template>
      ...
      <div class="space-y-4">
        ...
        <div class="flex gap-6">
          <div v-for="tier in tierData" :key="tier.label" class="flex items-center gap-2">
            ...
            <span class="text-sm text-gray-500 dark:text-gray-400">{{ tier.label }}: {{ tier.value }}</span>
          </div>
        </div>
      </div>
    </UCard>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <UCard>
        ...
        <div class="space-y-2">
          <UButton to="/app/admin/users" label="Manage Users" icon="i-heroicons-users" variant="ghost" color="neutral" block class="justify-start" />
          <UButton to="/app/admin/feedback" label="View Feedback" icon="i-heroicons-inbox" variant="ghost" color="neutral" block class="justify-start" />
          <UButton to="/app/admin/announcements" label="Post Announcement" icon="i-heroicons-megaphone" variant="ghost" color="neutral" block class="justify-start" />
        </div>
      </UCard>
      ...
      <UCard>
        ...
        <div class="space-y-3">
          ...
          <div class="flex items-center justify-between">
            ...
            <span class="text-sm text-gray-900 dark:text-white">{{ health?.checks?.database?.latency ?? '...' }}ms</span>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-sm text-gray-500 dark:text-gray-400">Uptime</span>
            <span class="text-sm text-gray-900 dark:text-white">{{ health?.uptime ? Math.floor(health.uptime / 3600) + 'h' : '...' }}</span>
          </div>
        </div>
      </UCard>
      ...
      <UCard>
        ...
        <div class="space-y-3">
          ...
          <div v-if="health?.checks?.stripe?.latency" class="flex items-center justify-between">
            ...
            <span class="text-sm text-gray-900 dark:text-white">{{ health.checks.stripe.latency }}ms</span>
          </div>
    ...
    <UCard v-if="usageStats?.users?.length">
      <template #header>
        <div class="flex items-center justify-between">
          <h3 class="text-base font-semibold text-gray-900 dark:text-white">Top API Users (7 days)</h3>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          ...
          <tbody>
            <tr v-for="u in usageStats.users" :key="u.userId" class="border-b border-gray-100 dark:border-gray-800/50">
              <td class="py-2">
                <p class="font-medium text-gray-900 dark:text-white text-xs">{{ u.name || u.email }}</p>
                <p v-if="u.name" class="text-xs text-gray-400">{{ u.email }}</p>
              </td>
              <td class="py-2 text-right font-mono">
                <span :class="u.avgDaily > (usageStats.globalAvgDaily || 1) * 2 ? 'text-red-500 font-bold' : 'text-gray-900 dark:text-white'">
